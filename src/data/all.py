...
class Preprocessor(data.Dataset):
    def __init__(self, cfg, listIDs, data_dict):
        ...
        self.shot_boundary_range = range(-cfg.shot_num // 2 + 1,
                                         cfg.shot_num // 2 + 1)
    ...
    def __getitem__(self, index):
        ...
        if isinstance(ID_list, (tuple, list)):
            ...
            for ID in ID_list:
                place_feat, cast_feat, act_feat, aud_feat, label = self._get_single_item(
                    ID)
...
# place_feat and label318 are needed
def data_pre_one(cfg, imdbid, acts_dict, casts_dict, annos_dict,
                 annos_valid_dict):
    data_root = cfg.data_root
    label_fn = osp.join(data_root, 'label318')  # ./data/label318
    place_feat_fn = osp.join(data_root,
                             'place_feat')  # specific folder for place feat
    win_len = cfg.seq_len + cfg.shot_num  # 10+4 for window size (?) - 1

    # original: ./data/place/{imdbid}.npy
    # unpack to: ./data/place/{imdbid}/shot_{shot_num}.npy
    files = os.listdir(osp.join(place_feat_fn, imdbid))
    all_shot_place_feat = [int(x.split('.')[0].split('_')[1]) for x in files
                           ]  # shot number list, e.g. 2430 shots * 2048

    anno_fn = '{}/{}.txt'.format(
        label_fn, imdbid)  # ./data/label318/xxx.txt, also 2430 lines
    anno_dict = get_anno_dict(anno_fn)  # scene transit 0 or 1: 0044 0
...
# multiprocessing's Manager() and Process() needed
def data_pre(cfg):
    data_root = cfg.data_root
    ...
    jobs = [
        Process(target=data_pre_one,
                args=(cfg, imdbid, acts_dict_raw, casts_dict_raw,
                      annos_dict_raw, annos_valid_dict_raw))
...
def main():
    ...
    imdbidlist_json, annos_dict, annos_valid_dict, casts_dict, acts_dict = data_pre(
        cfg)
        ...
        for index, _ in enumerate(listIDs):
            ...
            for ID in ID_list:
                if _get_single_item(ID, cfg, data_dict) is None:
                    continue
                place_feat, cast_feat, act_feat, aud_feat, label = _get_single_item(
                    ID, cfg, data_dict)
                place_feats.append(place_feat)
                cast_feats.append(cast_feat)
                act_feats.append(act_feat)
    ...
    for batch_idx, (data_place, data_cast, data_act, data_aud,
        ...
        print(data_place.shape, data_cast.shape, data_act.shape,
              data_aud.shape, target.shape)
