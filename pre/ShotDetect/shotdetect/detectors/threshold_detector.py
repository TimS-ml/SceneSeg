...
class ThresholdDetector(shotDetector):
    ...
    def frame_under_threshold(self, frame):
        ...
        num_pixel_values = float(frame.shape[0] * frame.shape[1] *
                                 frame.shape[2])
        ...
        while curr_frame_row < frame.shape[0]:
            ...
            curr_frame_amt += int(
                numpy.sum(frame[curr_frame_row:curr_frame_row +
                                self.block_size, :, :] > self.threshold))
    ...
    def process_frame(self, frame_num, frame_img):
        ...
        if (self.stats_manager is not None
                and self.stats_manager.metrics_exist(frame_num,
                                                     self._metric_keys)):
            frame_avg = self.stats_manager.get_metrics(frame_num,
                                                       self._metric_keys)[0]
        else:
        ...
        if self.last_frame_avg is not None:
            if self.last_fade['type'] == 'in' and self.frame_under_threshold(
                    frame_img):
                # Just faded out of a shot, wait for next fade in.
                self.last_fade['type'] = 'out'
                self.last_fade['frame'] = frame_num
            elif self.last_fade[
                    'type'] == 'out' and not self.frame_under_threshold(
                        frame_img):
                ...
                f_split = int(
                    (f_in + f_out + int(self.fade_bias * (f_in - f_out))) / 2)
