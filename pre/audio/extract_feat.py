...
def run_mp42wav(args, video_id, shot_id):
    source_movie_fn = osp.join(args.source_video_path, video_id,
                               "{}.mp4".format(shot_id))
    out_video_fn = osp.join(args.save_wav_path, video_id,
                            "{}.wav".format(shot_id))
    if not args.replace_old and osp.exists(out_video_fn):
        return 0
    call_list = ['ffmpeg']
    call_list += ['-v', 'quiet']
    call_list += ['-i', source_movie_fn, '-f', 'wav']
    call_list += ['-map_chapters', '-1']  # remove meta stream
    call_list += [out_video_fn]
    subprocess.call(call_list)
    if not osp.exists(out_video_fn):  # if not converted yet
...
def run_wav2stft(args, video_id, shot_id):
    ...
    feat_path = osp.join(args.save_stft_path, video_id,
                         '{}.npy'.format(shot_id))
    if args.replace_old and osp.exists(feat_path):
        return 0
    data, fs = librosa.core.load(osp.join(args.save_wav_path, video_id,
                                 '{}.wav'.format(shot_id)),
                                 sr=16000)
    ...
    # Returns a complex-valued matrix D 
...
def main(args):
    ...
    for video_id in video_list:
        shot_id_mp4_list = sorted(
            os.listdir(osp.join(args.source_video_path, video_id)))
        for shot_id_mp4 in shot_id_mp4_list:
            parallel_num += 1

    pool = multiprocessing.Pool(processes=args.num_workers)
    for video_id in video_list:
        shot_id_mp4_list = os.listdir(
            osp.join(args.source_video_path, video_id))
        ...
        for shot_id_mp4 in shot_id_mp4_list:
            ...
            # pdb.set_trace()
            pool.apply_async(run, (args, video_id, shot_id),
                             callback=call_back)
    pool.close()
    pool.join()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--replace_old',
                        action="store_true",
                        help='rewrite exisiting wav and feature')
    ...
    parser.add_argument('--list_file',
                        type=str,
                        default=osp.join(data_root, 'meta/list_test.txt'),
                        help='The list of videos to be processed,\
                        in the form of xxxx0.mp4\nxxxx1.mp4\nxxxx2.mp4\n \
                                     or xxxx0\nxxxx1\nxxxx2\n')
    parser.add_argument('--source_video_path',
                        type=str,
                        default=osp.join(data_root, "shot_split_video"))
    parser.add_argument('--save_wav_path',
                        type=str,
                        default=osp.join(data_root, "aud_wav"))
    parser.add_argument('--save_stft_path',
                        type=str,
                        default=osp.join(data_root, "aud_feat"))
