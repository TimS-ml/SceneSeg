...
def getIntersection(interval_1, interval_2):
    assert interval_1[0] < interval_1[
        1], "start frame is bigger than end frame."
    assert interval_2[0] < interval_2[
        1], "start frame is bigger than end frame."
...
def get_mAP_seq(loader, gts_raw, preds_raw):
    ...
    for movie in movies:
        index = np.where(imdbs == movie)[0]
        ap = average_precision_score(np.nan_to_num(gts[index]),
                                     np.nan_to_num(preds[index]))
        mAP.append(round(ap, 2))
    return np.mean(mAP), np.array(mAP)
...
def save_pred_seq(cfg, loader, gts, preds):
    ...
    for threshold in np.arange(0, 1.01, 0.05):
        pred_fn = osp.join(cfg.logger.logs_dir,
                           'pred/pred_{:.2f}.txt'.format(threshold))
        n = min(len(loader.dataset.listIDs),
                len(gts) // cfg.seq_len,
                len(preds) // cfg.seq_len)
...
def pred2scene(cfg, threshold=0.5):
    ...
    print('...scene dict with the information of each scene is saved in {}'.
          format(osp.join(cfg.logger.logs_dir, "pred_scene.json")))
